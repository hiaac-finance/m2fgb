...
def do_learning(
    ...
    if not use_input_commands and display_plots:
        warnings.warn(
            "WARNING: use_input_commands is set to False. "
    ...
    if save_models:
        warnings.warn(
            "WARNING: save_models is set to True but this has no default functionality. This will write "
    ...
    if relaxed and gamma == 0.0:
        warnings.warn(
            "WARNING: Running a relaxed simulation with gamma = 0.0 which will likely be infeasible."
            "\nTo run an unrelaxed simulation, please set the relaxed flag to False."
        )

    if not (relaxed or equal_error) and error_type in [
        "FP",
        "FN",
        "FP-Log-Loss",
        "FN-Log-Loss",
        ...
        warnings.warn(
            f"WARNING: Running an unconstrained simulation with {error_type} error type. \n"
            f"In this setting, the minimax solution w.r.t. {error_type} is to always predict "
            f"{label_type} labels, regardless of input."
        )

    if error_type.endswith("Log-Loss") and model_type not in [
        "LogisticRegression",
        "MLPClassifier",
    ]:
        raise Exception(
            "ERROR: Log-Loss error type is only supported for Logistic Regression."
        )

    if equal_error and model_type != "PairedRegressionClassifier":
        warnings.warn(
            "WARNING: Equal error rates is only supported for PairedRegressionClassifier "
    ...
    numgroups = np.array(
        [np.size(np.unique(grouplabels[i])) for i in range(num_group_types)]
    )
    ...
    model_classes = {
        ...
        "PairedRegressionClassifier": PairedRegressionClassifier,
        "Perceptron": Perceptron,
        "MLPClassifier": MLPClassifier,
        "Ridge": Ridge,
    }

    regression_models = ["LinearRegression", "MLPRegresor", "Ridge"]
    classification_models = [
        "LogisticRegression",
        "Perceptron",
        "PairedRegressionClassifier",
        "MLPClassifier",
    ]
    try:
        ...
        if model_type in classification_models:
            ...
            try:
                ...
                raise ValueError(
                    "Binary input labels y for classification must be encoded"
    ...
    do_validation = (
        test_size != 0.0
    )  # Stores a boolean flag on whether or not we are doing validation
    if do_validation:
        ...
        X_train, X_test, y_train, y_test, grouplabels_train, grouplabels_test = (
            create_validation_split(
                X, y, grouplabels, test_size, random_seed=random_split_seed
            )
        )
    else:
    ...
    if do_validation:
        ...
        for i in range(num_group_types):
            val_groupsize[i], val_index[i] = create_index_array(
                numgroups[i], grouplabels_test[i]
            )

    # Instatiate all error arrays
    errors = np.zeros(
        (numsteps, numsamples)
    )  # Stores error for each member of pop for each round
    # Store errors for each groups over rounds both for individual model and aggregate mixture
    grouperrs, agg_grouperrs = create_group_error_arrays(
        num_group_types, numsteps, numgroups
    )
    if do_validation:
        val_errors = np.zeros((numsteps, val_numsamples))
        val_grouperrs, val_agg_grouperrs = create_group_error_arrays(
            num_group_types, numsteps, numgroups
        )
    ...
    if model_type in classification_models:
        ...
        if error_type not in [
            ...
            raise Exception(
                f"ERROR: Unsupported classification error type: {error_type}"
            )
    else:
        # NOTE: Currently, the only supported error type for regression is MSE.
        if error_type != "MSE":
            warnings.warn(
                f"WARNING: Error type {error_type} is not usable in regression settings. \
            Automatically changing error type to `MSE` and continuing..."
            )
        error_type = pop_error_type = (
            "MSE"  # Rename the 'total' error type to MSE in regression case
        )

        if equal_error:
            warnings.warn(
                "WARNING: Equal error is not supported for regression. Returning minmax solution instead."
            )
            equal_error = False
    ...
    if (
        model_type in regression_models
    ):  # If we have a regression model, no additional error types make sense
        extra_error_types = set()
    elif error_type in [
        "FP",
        "FN",
    ]:  # If we are in FP/FN setting, then we always need to use Total error for pop
        ...
        extra_error_types.add("0/1 Loss")
        extra_error_types.add(
            error_type.split("-")[0]
        )  # Adds FP/FN for FP-Log-Loss/FN-Log-Loss
        pop_error_type = "0/1 Loss"
    ...
    prev_avg_sampleweights = np.zeros(
        numsamples
    )  # Will store previous round sampleweights for convergence detection
    # Fill the weight arrays as necessary
    for i in range(num_group_types):
        p[i] = (
            groupsize[i] / numsamples
        )  # Compute each groups proportion of the population
        groupweights[i][0] = p[i]
        sampleweights[i] = (
            np.ones(numsamples) / numsamples
        )  # Initialize sample weights array to uniform
    ...
    if verbose:
        print(
            f"Starting simulation with the following paramters: \n"
    ...
    for t in range(1, numsteps):
        ...
        if (
            t > 3
            and max(abs(avg_sampleweights - prev_avg_sampleweights))
        ...
        if model_type == "LogisticRegression":
            with warnings.catch_warnings():
                ...
                # try:
                modelhat = model_class(
                    max_iter=max_logi_iters,
                    tol=tol,
            ...
            modelhat = model_class(regressor_class=LinearRegression).fit(
                X_train, y_train, avg_sampleweights
            )
        elif (
            model_type == "MLPClassifier"
        ):  # Pytorch's MLP wrapped with our custom class to work with the interface
            hidden_sizes = [numdims] + list(
                map(
                    lambda x: x if np.floor(x) == x else int(np.floor(x * numdims)),
                    hidden_sizes,
                )
            )
            modelhat = MLPClassifier(
                hidden_sizes, lr=lr, momentum=momentum, weight_decay=weight_decay
            ...
            modelhat = model_class(
                ...
                solver=logistic_solver,
                tol = tol,
            ).fit(X_train, y_train, sample_weight=avg_sampleweights)
        else:  # Linear Regression or Perceptron
            modelhat = model_class().fit(
                X_train, y_train, sample_weight=avg_sampleweights
            )
        ...
        if model_type in regression_models:
            ...
            compute_model_errors(
                modelhat, X_train, y_train, t, errors, error_type, penalty, C
            )
            ...
            if do_validation:
                compute_model_errors(
                    modelhat, X_test, y_test, t, val_errors, error_type, penalty, C
                )
                for err_type in extra_error_types:
                    compute_model_errors(
                        modelhat,
        ...
        for i in range(num_group_types):
            ...
            if do_validation:
                update_group_errors(
                    ...
                    val_agg_grouperrs[i],
                    val_index[i],
                    val_groupsize_err_type[i],
                )

            # Weight update type depends on relaxed or not
            if relaxed:  # Projected Gradient descent
                lambdas[i][t] = np.maximum(
                    0, lambdas[i][t - 1] + (eta * (grouperrs[i][t] - gamma))
                )
                ...
                if equal_error:  # GD where errors are pushed to mean error
                    poperrs = errors[
                        t
                    ]  # if model_type in regression_models else specific_errors['Total'][t]
                    mean_poperrs = sum(poperrs) / numsamples
                    lambdas[i][t] = lambdas[i][t - 1] + (
                        eta * (grouperrs[i][t] - mean_poperrs)
                    )
                    groupweights[i][t] = (
                        p[i] * (1 - np.sum(lambdas[i][t])) + lambdas[i][t]
                    )
                else:  # Exponential Weights, Minmax algorithm
                    groupweights[i][t] = np.multiply(
                        groupweights[i][t - 1], np.exp(eta * (grouperrs[i][t]))
                    )
                    groupweights[i][t] = groupweights[i][t] / np.sum(
                        groupweights[i][t]
                    )  # normalize

            # Translate groups weights to sample weights for Learner
            for g in range(0, numgroups[i]):
                if (relaxed or equal_error) and error_type in [
                    "FP",
                    "FN",
                    "FP-Log-Loss",
                    "FN-Log-Loss",
                    ...
                    baseline_weight = (
                        1 / numsamples
                    )  # if not use_obj_as_constraint else lambda_obj[i] / numsamples
                    ...
                    if error_type.startswith("FP"):
                        ...
                        if groupsize_neg[i][g] > 0:
                            weight_per_neg_sample = (
                                baseline_weight + lambdas[i][t, g] / groupsize_neg[i][g]
                            )
                    else:  # 'error_type' == 'FN'
                        if groupsize_pos[i][g] > 0:
                            weight_per_pos_sample = (
                                baseline_weight + lambdas[i][t, g] / groupsize_pos[i][g]
                            )
            ...
            if equal_error and model_type != "PairedRegressionClassifier":
                ...
                if weight_to_add > 0:
                    sampleweights[i] = (
                        sampleweights[i] + weight_to_add
                    )  # np.maximum(0, -np.min(sampleweights))
    ...
    agg_poperrs = compute_mixture_pop_errors(
        specific_errors[pop_error_type], total_steps
    )
    if do_validation:
        val_agg_poperrs = compute_mixture_pop_errors(
            val_specific_errors[pop_error_type], total_steps
        )
    ...
    if display_plots or save_plots:
        ...
        model_string = (
            f"\n {model_type}"
            + loss_string
            + (" for Equal-Error" if equal_error else "")
            + (f"\n Gamma={gamma}" if relaxed else "")
        )

        # Combine groups names with their sizes for display when plotting
        group_names_and_sizes_list = get_group_names_and_sizes_list(
            group_names, groupsize, num_group_types
        )
        # Generate bonus plots as necessary
        stacked_bonus_plots = create_stacked_bonus_plots(
            num_group_types,
            extra_error_types,
            numgroups,
            specific_errors,
        ...
        if do_validation:
            val_group_names_and_sizes_list = get_group_names_and_sizes_list(
                group_names, val_groupsize, num_group_types
            )
            val_stacked_bonus_plots = create_stacked_bonus_plots(
                num_group_types,
                extra_error_types,
                numgroups,
                val_specific_errors,
            ...
            do_plotting(
                ...
                use_input_commands,
                total_steps,
                val_group_names_and_sizes_list,
                group_types,
                show_legend,
                error_type,
                data_name,
    ...
    for i in range(num_group_types):
        final_max_group_error[i] = np.max(
            agg_grouperrs[i][-1]
        )  # This is the minimum gamma we think is feasible
        highest_gamma[i] = compute_highest_gamma(agg_poperrs, agg_grouperrs[i], relaxed)

    if do_validation:
        val_max_grp_err = np.max(
            val_agg_grouperrs[:][-1]
        )  # This is the minimum gamma we think is feasible
        val_pop_err = val_agg_poperrs[
            -1
        ]  # max groups error when optimizing for pop error

        # For now, we are taking the max over all types of groups
        return (
            max(final_max_group_error),
            max(highest_gamma),
            agg_poperrs[1],
            agg_grouperrs,
            agg_poperrs,
        ...
        if relaxed:
            margin_of_error = (
                0.001  # Allows for a small margin of error for 'unfeasible' gammas
            )
            if gamma + margin_of_error < np.max((agg_grouperrs[:][-1])):
                warnings.warn(
                    f"WARNING: Desired gamma value may not be feasible with margin of error: "
                    f"{margin_of_error}. \n"
...
def create_bonus_plots(
    extra_error_types, numgroups, specific_errors, index, groupsize, total_steps
):
    """
    ...
    for err_type in extra_error_types:
        ...
        grp_errs = compute_mixture_group_errors(
            numgroups, specific_errors[err_type], index, groupsize, total_steps
        )
        pop_errs = compute_mixture_pop_errors(
            specific_errors[pop_err_type], total_steps
        )
        bonus_plots.append((err_type, grp_errs, pop_errs, pop_err_type))
    return bonus_plots


def create_stacked_bonus_plots(
    num_group_types,
    extra_error_types,
    numgroups,
    specific_errors,
    ...
    for err_type in extra_error_types:
        ...
        grp_errs = compute_stacked_mixture_group_errors(
            num_group_types,
            numgroups,
            specific_errors[err_type],
            index,
...
def compute_model_errors(modelhat, X, y, t, errors, error_type, penalty="none", C=1.0):
    ...
    """
    yhat = modelhat.predict(
        X
    ).ravel()  # Compute predictions for the newly trained model
    ...
    if error_type == "MSE":
        ...
        error_type == "Log-Loss"
    ):  # Log loss, the convex surrogate used by logistic regression
        errors[t, :] = compute_logloss(y, modelhat.predict_proba(X))
    elif (
        error_type == "FP-Log-Loss"
    ):  # Computes the log loss, but replaces loss with 0 unless an instance was a FP
    ...
    if penalty in ["l1", "l2"] and C > 1e15:
        errors[t, :] += compute_regularization_penalty(modelhat.coef_, penalty, C) * (
            0.5 if penalty == "l2" else 1.0
        )


def compute_regularization_penalty(coef, penalty, C):
    warnings.warn(
        "WARNING: Regularization term is being applied to log-loss. If you did not intend, this, please set"
    ...
    if penalty == "l1":
        ...
        return (
            0.5 * np.sum(np.power(coef, 2)) / C
        )  # 0.5 coefficient matches sklearn's calculation
    else:
        raise Exception(f"Unsupported penalty type: {penalty}")


def update_group_errors(
    numgroups, t, errors, grouperrs, agg_grouperrs, index, groupsize
):
    """
    ...
    for g in range(0, numgroups):
        ...
        grouperrs[t, g] = np.sum(errors[t, index[g]]) / groupsize[g]
        # Compute the aggregate-model average groups error with DP
        agg_grouperrs[t, g] = (agg_grouperrs[t - 1, g] * ((t - 1) / t)) + (
            grouperrs[t, g]
        ) / t
...
def compute_mixture_group_errors(numgroups, errors, index, groupsize, total_steps=None):
    ...
    agg_grouperrs = np.zeros(
        (numsteps, numgroups)
    )  # Mixture model errors for each groups over rounds
...
def compute_stacked_mixture_group_errors(
    num_group_types, numgroups, errors, index, groupsize, total_steps=None
):
    """
    ...
    for i in range(num_group_types):
        agg_grouperrs_list.append(
            compute_mixture_group_errors(
                numgroups[i], errors, index[i], groupsize[i], total_steps=total_steps
            )
        )
...
def compute_logloss(y_true, y_pred_proba, eps=1e-15):
    ...
    return (y_true * (-np.log(y_pred_proba))) + (
...
def compute_highest_gamma(agg_poperrs, agg_grouperrs, relaxed):
    ...
    if not relaxed:
        try:
            ...
            best_index = min(best_indices, key=lambda x: max(agg_grouperrs[x]))[
                0
            ]  # unpack the array
            highest_gamma = max(
                agg_grouperrs[best_index]
            )  # error of the max error groups when pop error is minmized
        except ValueError:
            highest_gamma = max(
                agg_grouperrs[1]
            )  # Set to initial max groups error if something goes wrong
    else:
        highest_gamma = 0.0
...
def rescale_feature_matrix(X):
    ...
    for col in range(X.shape[1]):
        magnitude = np.floor(
            np.log10(max(abs(X[:, col])))
        )  # Finds the order of magnitude of max val
        X[:, col] /= np.power(
            10, magnitude
        )  # Put all feature values in the range [-1, 1]
